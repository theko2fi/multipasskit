...
from multipasskit.sdk.decorators import *

class MultipassVM_by_SDK:
    ...
    def info(self):
        ...
        if(exitcode != 0):
            ...
            if f'instance "{self.vm_name}" does not exist' in stderr_cleaned:
                raise NameError(f"Multipass info command failed: {stderr_cleaned[1]}")
            if "Socket error" in stderr.decode(encoding="utf-8"):
                raise SocketError(f"Multipass info command failed: {stderr_cleaned[0]}")
            else:
                raise Exception(f"Multipass info command failed: {stderr.decode(encoding='utf-8')}")
    ...
    def shell(self):
        ...
        f"ttyd multipassh shell {self.vm_name}",
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        shell=True,
        )
...
class MultipassClientSDK:
    ...
    def launch(self, vm_name=None, cpu=1, disk="5G", mem="1G", image=None, cloud_init=None):
        ...
        return vm_name
        #return MultipassVM_by_SDK(vm_name, self.cmd)
        ...
        if(not exitcode == 0):
            for error_msg in stderr_cleaned:
                if 'is already mounted' in error_msg:
                    raise MountExistsError(f"Multipass mount command failed: {error_msg}")
        ...
        if(not exitcode == 0):
            for error_msg in stderr_cleaned:
                if 'is not mounted' in error_msg:
                    raise MountNonExistentError(f"Multipass umount command failed: {error_msg}")
    ...
    def get(self, key=None, keys=False):
        cmd = [self.cmd, "get"]
        if keys:
            cmd.append("--keys")
        if key:
